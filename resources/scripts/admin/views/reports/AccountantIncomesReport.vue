...
const reportTypes = ref([
  {
    label: t('reports.accountant.incomes.type.by_invoice_income'),
    value: 'By Invoice'
  },
  {
    label: t('reports.accountant.incomes.type.by_invoice_customer'),
    value: 'By Customer'
  },
  {
    label: t('reports.accountant.incomes.type.by_invoice_country'),
    value: 'By Country'
  }
])
const selectedType = ref('By Invoice')
let range = ref(new Date())
let url = ref(null)
let accountantIncomeSiteURL = ref(null)
let accountantCustomerSiteURL = ref(null)
...
const customerDateRangeUrl = computed(() => {
  return `${accountantIncomeSiteURL.value}?from_date=${moment(
    formData.from_date
...
const supplierGroupDateRangeUrl = computed(() => {
  return `${accountantCustomerSiteURL.value}?from_date=${moment(
    formData.from_date
  ).format('YYYY-MM-DD')}&to_date=${moment(formData.to_date).format(
    'YYYY-MM-DD'
  )}`
...
onMounted(() => {
  accountantIncomeSiteURL.value = `/reports/accountant/incomes/invoice/${getSelectedCompany.value.unique_hash}`
  accountantCustomerSiteURL.value = `/reports/accountant/incomes/customer/${getSelectedCompany.value.unique_hash}`
  accountantCountrySiteURL.value = `/reports/accountant/incomes/country/${getSelectedCompany.value.unique_hash}`
  getInitialReport()
})
...
async function getInitialReport() {
  if (selectedType.value === 'By Invoice') {
