...
const selectedRange = ref(dateRange[5])
const reportTypes = ref([
  {
    label: t('reports.accountant.expenses.type.by_invoice_expense'),
    value: 'By Expense'
  },
  {
    label: t('reports.accountant.expenses.type.by_invoice_supplier'),
    value: 'By Supplier'
  },
  {
    label: t('reports.accountant.expenses.type.by_invoice_country'),
    value: 'By Country'
  }
])
const selectedType = ref('By Expense')
let range = ref(new Date())
let url = ref(null)
let accountantExpenseSiteURL = ref(null)
let accountantSupplierSiteURL = ref(null)
let accountantCountrySiteURL = ref(null)

let formData = reactive({
  from_date: moment().subtract(1, 'month').startOf('month').format('YYYY-MM-DD').toString(),
  to_date: moment().subtract(1, 'month').endOf('month').format('YYYY-MM-DD').toString(),
})
...
const customerDateRangeUrl = computed(() => {
  return `${accountantExpenseSiteURL.value}?from_date=${moment(
    formData.from_date
...
const supplierGroupDateRangeUrl = computed(() => {
  return `${accountantSupplierSiteURL.value}?from_date=${moment(
    formData.from_date
  ).format('YYYY-MM-DD')}&to_date=${moment(formData.to_date).format(
    'YYYY-MM-DD'
  )}`
...
const countryGroupDateRangeUrl = computed(() => {
  return `${accountantCountrySiteURL.value}?from_date=${moment(
    formData.from_date
  ).format('YYYY-MM-DD')}&to_date=${moment(formData.to_date).format(
    'YYYY-MM-DD'
  )}`
...
onMounted(() => {
  accountantExpenseSiteURL.value = `/reports/accountant/expenses/expense/${getSelectedCompany.value.unique_hash}`
  accountantSupplierSiteURL.value = `/reports/accountant/expenses/supplier/${getSelectedCompany.value.unique_hash}`
  accountantCountrySiteURL.value = `/reports/accountant/expenses/country/${getSelectedCompany.value.unique_hash}`
  getInitialReport()
})
...
async function getInitialReport() {
  if (selectedType.value === 'By Expense') {
    url.value = customerDateRangeUrl.value
    return true
  }
  if (selectedType.value === 'By Supplier') {
    url.value = supplierGroupDateRangeUrl.value
    return true
  }
  if (selectedType.value === 'By Country') {
    url.value = countryGroupDateRangeUrl.value
    return true
  }
