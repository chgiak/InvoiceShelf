...
<head>
    <title>@lang('pdf_accountant_incomes_invoice_report_label')</title>
    <style type="text/css">
        body {
...
<body>
    <div class="sub-container">
        <table class="report-header">
            ...
            <tr>
                <td colspan="2">
                    <p class="sub-heading-text">@lang('pdf_accountant_incomes_invoice_report_label')</p>
                </td>
            </tr>
        </table>
        ...
        <table class="invoice-detail-table">
            <thead>
                <tr>
                    <th style="width: 15%">@lang('pdf_customer')</th>
                    ...
                    <th style="width: 10%">@lang('pdf_payment_status')</th>
                    <th style="width: 10%">@lang('pdf_payment_label')</th>
                    <th style="width: 11%">@lang('pdf_price_label')</th>
                    <th style="width: 10%">@lang('pdf_price_vat_label')</th>
                    <th style="width: 10%">@lang('pdf_subtotal')</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $invoiceData['vat_id'] }}</td>
                    <td>{{ Carbon\Carbon::parse($invoiceData['invoice_date'])->format($company->date_format ?: 'Y-m-d') }}</td>
                    <td>{{ $invoiceData['invoice_number'] }}</td>
                    <td>
                        @if($invoiceData['payment_status'] === 'PAID')
                            <span class="status-paid">@lang('pdf_paid')</span>
                        @elseif($invoiceData['payment_status'] === 'UNPAID')
                            <span class="status-unpaid">@lang('pdf_unpaid')</span>
                        @else
                            <span class="status-partially-paid">@lang('pdf_partially_paid')</span>
                        @endif
                    </td>
                    <td>{{ $invoiceData['payment_method'] }}</td>
                    <td>{!! format_money_pdf($invoiceData['amount_without_vat'], $currency) !!}</td>
                    <td>{!! format_money_pdf($invoiceData['vat_amount'], $currency) !!}</td>
                    <td>{!! format_money_pdf($invoiceData['total_amount'], $currency) !!}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr class="total-row">
                    <td colspan="6">@lang('pdf_total')</td>
