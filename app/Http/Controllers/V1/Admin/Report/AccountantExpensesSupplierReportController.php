...
class AccountantExpensesSupplierReportController extends Controller
{
    /**
    ...
    public function __invoke(Request $request, $hash)
    {
        ...
        $groupedExpenses = $expenses->groupBy(function ($expense) {
            return $expense->customer ? $expense->customer->id : 'no-customer';
        })->map(function ($customerExpenses) {
            $customer = $customerExpenses->first()->customer;

            // Calculate totals for this customer
            $totalAmount = $customerExpenses->sum('base_amount');
            $expenseCount = $customerExpenses->count();
            ...
            return [
                'supplier' => $customer ? $customer->name : '-',
        ...
        $pdf = PDF::loadView('app.pdf.reports.accountant-expenses-supplier')
            ->setPaper('a4', 'landscape');

        if ($request->has('preview')) {
            return view('app.pdf.reports.accountant-expenses-supplier');
        }
